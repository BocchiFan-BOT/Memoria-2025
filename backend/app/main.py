...
# cameras CRUD (persist en JSON file via camera_manager)
@app.get("/cameras")
def get_cameras():
    return list_cameras()
...
async def post_cameras(payload: list):
    # payload is full list from frontend; replace disk copy
    # simple approach: clear and add
    from .camera_manager import _cameras, _processors
    # WARNING: this is simplistic; adapt as you need
...
def stream(cam_id: str):
    # returns multipart MJPEG from VideoProcessor.get_frame()
...
def report(cam_id: str):
    ...
    return FileResponse(csv_path, media_type="text/csv", filename=f"reporte_{cam_id}.csv")
